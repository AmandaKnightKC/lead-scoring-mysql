...
expected_columns = {
    ...
    "quotes": ["agent_name", "product_name", "created_date", "lead_id"],
    "applications": ["agent_id", "product_name", "submitted_timestamp", "lead_id"],
}
...
df_leads = None # to hold marketing_leads for filtering quotes and applications
...
for sheet_name, df in sheets.items():
    ...
    expected = expected_columns[ # df [key]
        sheet_name
    ]  # align dataframes to expected schema (extra columns will be ignored, missing columns will be filled with nulls)
    ...
    if sheet_name in ["quotes", "applications"] and df_leads is not None: # only load quotes/applications with valid lead_id in marketing_leads
        valid_lead_ids = set(df_leads["lead_id"]) #set contains unique lead_ids from marketing_leads
        df = df[df["lead_id"].isin(valid_lead_ids)] # filter out rows where lead_id is not in sheet_name
